...
int GetMotorPulse(int timx) {
	...
	switch(timx) {
		...
		case 3: 
			...
			break;
		default:
		// [BUGFIX] invalid timx (not 1 or 3) used to silently return 0, which hides
		// wiring / parameter mistakes during debugging. Now we print a warning.
		printf("GetMotorPulse: invalid timx=%d (expected 1 or 3)\r\n", timx);
		encoder_counter = 0;
	}  

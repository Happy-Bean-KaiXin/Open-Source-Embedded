...
void Straight_Trail_param(void) {
	...
	// [BUGFIX] NOTE: speed-loop gains are POSITIVE (intuitive: positive = drive forward).
	// The error is kept as the textbook Bias = target - reality (NOT reality - target).
	// Why this is required: the loop direction is set by the sign product
	//   P = (encoder_sign) x (Kp sign) x (error-form sign)
	// The original firmware (encoder negated, Kp negative, Bias=target-reality) gave
	// P = (-1)x(-1)x(-1) = -1 and drove FORWARD. We flipped the encoder to POSITIVE and
	// flipped Kp to POSITIVE; to keep P = -1 (same direction) the error form must stay
	// -1, i.e. Bias = target - reality. Using reality - target would make P = +1 and the
	// car runs backward -- sim-verified (see sim/speed_loop_sim.py). So: encoder +, Kp +,
	// textbook error => all intuitive AND still forward.
	Position.Kp = 30.00f;
	Position.Ki = 2.600f;
	Position.Kd = 0.0f;
	
	// [BUGFIX] Turn-loop gain is now POSITIVE (+1.2) for the same intuition as the
	// speed loop (positive = "normal" steering). Turn_PID_Realize takes the error
	// directly, so to keep the EXACT same steering direction as the original
	// (Kp=-1.2 with Bias=+TraceDate) we negate the input at the call site:
	//   (+1.2) * (-TraceDate)  ==  (-1.2) * (+TraceDate)  ->  identical turnpwm.
	// See ChangeTraceTurn() in control.c.
	Turn_PID.Kp = 1.2f;
	Turn_PID.Kd = 0.0f;
}
...
float Position_PID_Realize(float reality, float target) { 	
    ...
    Bias=target-reality;                            /* 计算偏差 (textbook form; with encoder + and Kp + this keeps the loop direction equal to the original firmware, P=-1) */

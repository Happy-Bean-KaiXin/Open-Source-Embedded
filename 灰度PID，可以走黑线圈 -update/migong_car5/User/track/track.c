...
//	static char PosFlag = 0;//定义传感器位置标志位，0认为传感器在黑线偏左位置，1认为小车在传感器偏右位置
	...
	// [BUGFIX] Comments on Right_AD/Left_AD were swapped; fixed to match variable names.
	// [NOTE] Sensor raw output has opposite polarity (high on white, low on black);
	//         invert via 4096-raw so black line = high value (matches thresholds/stop-line).
	Right_AD = -Get_Adc(5) + 4096;   // right sensor AD value
	Mid_AD   = -Get_Adc(4) + 4096;   // middle sensor AD value
	Left_AD  = -Get_Adc(1) + 4096;   // left sensor AD value
...
void GetParament(void)
{
	...
	// [BUGFIX] Same comment-swap fix as in GetTraceDate (comments now match variable names).
	Right_AD = -Get_Adc(5) + 4096;   // right sensor AD value
	Mid_AD   = -Get_Adc(4) + 4096;   // middle sensor AD value
	Left_AD  = -Get_Adc(1) + 4096;   // left sensor AD value
	...
	if(Left_AD>LeftMax)	
	{
		// [BUGFIX] compute SPAN from PREVIOUS peak (LeftMax) BEFORE updating it
	...
	{
		// [BUGFIX] compute SPAN from PREVIOUS peak (RightMax) BEFORE updating it
...
//		OLED_ShowStr(0, 2, (unsigned char* )buf, 1);
		...
		sprintf(buf, (const char* )"RightMax:%d ",RightMax);   // [BUGFIX] was printing MidMax instead of RightMax

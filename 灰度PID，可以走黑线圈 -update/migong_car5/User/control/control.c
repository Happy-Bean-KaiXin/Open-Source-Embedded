...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
	if(htim->Instance == TIM4){
		// 获取当前单位时间脉冲
		// [BUGFIX] encoder now read POSITIVE (removed the leading '-' on -GetMotorPulse).
		// Reality polarity is now +pulses, so Sigma_MotorPulse (and Distance) become
		// POSITIVE too (previously the distance sign was reversed).
		// NOTE: because the encoder sign flipped, pid.c must keep the TEXTBOOK error
		// Bias = target - reality (NOT reality - target) so that the loop direction
		// stays equal to the original firmware (sign product P = -1). With encoder +,
		// Kp + and Bias = target - reality the car still drives forward. Sim-verified
		// in sim/speed_loop_sim.py (scenario "FIX intuitive").
		...
		// [BUGFIX] cast to float first; integer division truncated distance to whole turns.
		Motor1.Distance = (float)Motor1.Sigma_MotorPulse/ACircleEncoder * pi * WheelDiameter;
		// [BUGFIX] cast to float first; integer division truncated distance to whole turns.
		Motor2.Distance = (float)Motor2.Sigma_MotorPulse/ACircleEncoder * pi * WheelDiameter;
...
int ChangeTraceTurn(int TraceDate)
{
	int pwm=0;
	int bias;
	// [BUGFIX] Turn_PID.Kp is now POSITIVE (+1.2). To keep the exact same steering
	// direction as the original (Kp=-1.2 with Bias=+TraceDate) we feed the NEGATED
	// sensor offset as the turn error: bias = -TraceDate.
	// Equivalent rearrangement: (+1.2)*(-TraceDate) == (-1.2)*(+TraceDate).
	bias = -TraceDate;
...
void TraceMove(int TraceDate,float TarSpeed)
{
	...
	// [BUGFIX] The shipped code lets the LEFT speed-loop read Motor2 and the RIGHT read Motor1.
		// TIM1->Motor1, TIM3->Motor2. Confirm against the schematic which physical wheel is on
		// TIM1/TIM3, then define MOTOR_LEFT_IS_MOTOR1 in control.h to swap to the correct mapping.
#ifdef MOTOR_LEFT_IS_MOTOR1
		spdpwml=ChangeSpeedMotorL(Motor1.Uinttime_MotorPulse, TarSpeed);
		spdpwmr=ChangeSpeedMotorR(Motor2.Uinttime_MotorPulse, TarSpeed);
#else

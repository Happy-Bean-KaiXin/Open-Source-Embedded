# -*- coding: utf-8 -*-
"""
中等差异区间仿真 v2 (fig5_moderate_case.png)
  - 新增"理想状态线"（纯浮点、无int截断，仅有死区约束的理论下限）
  - 修正标注框（解释平局情况）
  - 新增"终点误差物理含义"示意图（实际停点P'到目标点E的距离）
  - v2: 接入 analysis_utils —— 顶部目的横幅 + 自动换行解释区, 杜绝豆腐块/重叠
...
def main():
    ...
    # ---------- 主图（上半部分：曲线） ----------
    fig = plt.figure(figsize=(11, 24.0))
    # 顶部横幅占 y>0.93, 主图压到 0.55~0.90
    ax = fig.add_axes([0.08, 0.55, 0.88, 0.33])
    ...
    # ---- 下半部分：详细解释区（自动换行防重叠） ----
    au.add_banner(fig, "图5  中等差异区间终点误差对比",
                  "聚焦 ratio 0.1~10 这段“规则 vs 错选差距较小”的区间, 并叠加理想状态理论下限, "
                  "回答: 中等斜率下选错轴到底要不要紧?")
    au.add_explanation(fig, [
        ("【这张图在模拟什么】", True),
        "固定 dy=40px 不变, 让 dx 从 4px 连续扫到 400px(ratio=0.1~10, 对数 200 个点), "
        "逐点算出“按规则选轴”和“反着选轴”的终点误差曲线。",
        ("【三条线分别是什么】", True),
        "· [蓝实线] 固件按规则选轴(当前代码实际表现): |dx|>=|dy| 驱动X, 否则驱动Y;",
        "· [橙虚线] 固件反着选轴(故意违反规则的假想实验), 用来量化选错轴的代价;",
        "· [绿点划] 理想状态理论下限(纯 double 浮点+仅死区约束, 不做 int 截断), "
        "代表这套机制能达到的最好水平。",
        ("【怎么读】", True),
        "· 蓝线全程压在低位(大部分 0~5px) → 规则选轴误差始终很小;",
        "· 绿线几乎和蓝线重合、偶尔略低 → int 截断带来的额外误差仅 0~1px, "
        "主要误差来源是死区机制本身, 不是截断;",
        "· 橙线在两端(ratio 很大或很小时)爆炸上升(30px+), 中间(ratio≈1)接近蓝线;",
        "· 5 个标注点(圆点=规则, 三角=错选)落在对应曲线上, 可直接读出数值。",
        ("【中等差异区间的结论】", True),
        "· ratio<0.3 或 >3 (极端倾斜): 规则 vs 错选差距巨大(可达 30px/85mm) → 必须按规则;",
        "· 0.3~3 (中等差异, 如 dx=60/dy=40): 差距微小(0~4px/0~11mm) → 规则仍略优, 选错不致命;",
        "· ratio=1 (正对角): 完全对称, 差距为零 → 选谁都一样;",
        "· 右上角摘要框解释了“平局”真相: dx=40,dy=60 时两轴误差碰巧相等, 是整数截断的巧合, "
        "不代表选轴无所谓。",
        ("【工程结论】", True),
        "当前规则 |dx|>=|dy|→驱动X 在全部区间都是最优或不劣的选择, 且自动兜底极端情况;"
        "中等斜率区间即使偶尔选错, 代价也可接受, 但按规则仍是最稳策略。",
    ], y0=0.01, h=0.50, title="详细说明", fs=8.6, wrap_width=104)

    fig.savefig(os.path.join(out_dir, "fig5_moderate_case.png"), dpi=180, bbox_inches="tight",

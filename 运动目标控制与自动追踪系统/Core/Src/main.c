...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	Servo_Angle_Init();
	Servo_X_Angle_Set(0);
	Servo_Y_Angle_Set(0);  // 舵机复位初始化
	
	__HAL_UART_ENABLE_IT(&huart2, UART_IT_IDLE); //使能IDLE中断
  HAL_UART_Receive_DMA(&huart2,uart2_rx_buffer,BUFFER_SIZE);
	HAL_TIM_Base_Start_IT(&htim4);  // 开启基本定时器4
	HAL_TIM_Base_Start_IT(&htim3);  // 开启基本定时器3
//	HAL_UART_Receive_IT(&huart2,&rec,1);
  ...
  while (1)
  {
		USART_data_processing();  // 串口接收处理函数
		OLED_Proc();
		VOFA_Proc();
		Key_Proc();
